...
std::vector<prompt::PromptOption> load_from_paramters(rclcpp::Node::SharedPtr node, std::string plugin_name)
{
  ...
  for (const auto& key : model_option_keys)
  {
    ...
    opt.key = key;
    opt.type = node->declare_parameter(plugin_name + ".prompt_options." + key + ".type", rclcpp::ParameterValue(""))
                   .get<std::string>();

    // check if value is of type string
    if (opt.type == "string")
    {
      opt.value = node->declare_parameter(plugin_name + ".prompt_options." + key + ".value", rclcpp::ParameterValue(""))
                      .get<std::string>();
    }
    ...
    {
      bool value =
          node->declare_parameter(plugin_name + ".prompt_options." + key + ".value", rclcpp::ParameterValue(false))
              .get<bool>();
    ...
    {
      int value = node->declare_parameter(plugin_name + ".prompt_options." + key + ".value", rclcpp::ParameterValue(0))
                      .get<int>();
    ...
    {
      double value =
          node->declare_parameter(plugin_name + ".prompt_options." + key + ".value", rclcpp::ParameterValue(0))
              .get<double>();
      opt.value = std::to_string(value);
    }
